...
[3.2] POST-RELEASE FIX: Claude Code 2.1.246 negotiates 2025-11-25 AND advertises elicitation; on that combination ESCALATE returned the 2026-07-28 input_required result, the SDK rejected it ('Handler returned an invalid result') and the human was never asked (fail-closed, 0 sends). Handshake-era sessions with elicitation now get a standalone elicitation/create round trip; 3 new real-stdio legacy-mode tests (approve->1 send, decline->0, no elicitation->deny+approve id). Verified Claude Code's own client connects: 'claude mcp list' -> Connected.
```
...
- **Claude Code, verified end to end (post-release check, 2026-09-03):** `trilock init` wrapped a real `.mcp.json`; Claude Code 2.1.246's own client reported `✔ Connected` via `claude mcp list`; a probe server recorded that it negotiates `2025-11-25` and advertises `elicitation`. That combination exposed a real gap in 3.2 - an `ESCALATE` on a handshake-era session returned the `2026-07-28` `input_required` shape, which the SDK rejected, so the human was never asked (it failed closed: zero sends). Fixed with a standalone `elicitation/create` round trip, tested over real stdio for approve / decline / no-elicitation. Claude Code was not driven interactively (the project-scoped server needs a one-time in-app trust click).
...
- **`integrity` policy** + `session_untrusted` condition: every external action after untrusted input escalates - the two-leg integrity class behind the residual AgentDojo 0.135. Measured as a fifth configuration AgentDojo ASR **0.022** vs 0.117 for strict/dataflow, oracle utility 0.412 / 0.825 with a human; the residual is one slack task whose goal is a `get_webpage` GET of the attacker's URL.
- **Benchmark honesty fix.** AgentDojo judges `slack.injection_task_5` from the trace of emitted tool calls; the harness had put *refused* calls in that trace, so blocked attacks scored as successes (21/21 pairs under strict, 8 with nothing executed). Refused calls are now recorded without a tool call. Headline strict/dataflow 0.135 -> 0.123, integrity 0.050 -> 0.033. Test: `tests/unit/test_agentdojo_harness.py`.
- **Slack classification corrected on provenance grounds.** `get_channels`/`get_users_in_channel` return names written by other members - one AgentDojo injection vector is a channel name - and were classified trusted. Now untrusted. Headline 0.123 -> **0.117**, integrity 0.033 -> **0.022**; slack oracle utility 0.476 -> 0.333 (0.825 overall with a human). Rows carry `attack_successes_by_injection_task` so RESULTS.md derives what survives instead of asserting it.
